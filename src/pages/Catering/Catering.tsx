...
function Catering() {
  const catering = useStore((state) => state.catering);
  ...
  const handleClick = (id: number) => {
    setSelectedProduct(catering[id]);
  };
      ...
      <div className={s.store__wrapper}>
        ...
        <div className={s.store__list}>
          <div className={s.store__content}>
            ...
            <h2 className={s.list__header}>{t('store.catering')}</h2>
            <div className={s.list__breakline} />
            <p className={s.list__description}>
              {t('store.cateringDescription')}
            </p>
          </div>
          <div className="flex flex-wrap">
            {catering &&
              catering.map((product, index) => (
                <StoreCard
                  key={index}
                  title={product.title}
                  price={product.price}
